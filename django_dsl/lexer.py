...
def t_error(t):
    try:
        raise CompileException("Cannot make sense of char: %s (ord: %i)" % t.value[0], ord(t.value[0]))
    except TypeError:
        raise CompileException(f"Compilation error near token {t.value} in line {t.lineno}, position {t.lexpos}")
